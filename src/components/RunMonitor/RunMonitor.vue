<template>
  <div id="myMap">
    <div class="myMap-search" @keydown="searchVillages($event,1)">
      ...
      <!-- ------------------------------静态信息窗口-------------------------------------------- -->
      <el-dialog :title="chioseVillage.name" class="basisInfoDialog" :visible.sync="toolsDialog.infoTab" width="670px" @close="toolsDialog.dialogDestroy=false">
          <village-info-dialog ref="basisInfo" :chiose-village="chioseVillage" v-if="toolsDialog.dialogDestroy"></village-info-dialog>
      </el-dialog>

       <!-- ------------------------------维修记录窗口-------------------------------------------- -->
      <el-dialog :title="chioseVillage.name" class="maintainDialog" :visible.sync="toolsDialog.maintain" width="870px"   @close="toolsDialog.dialogDestroy=false">
        <maintain-info ref="maintainInfo" :villageId="chioseVillage.id" v-if="toolsDialog.dialogDestroy"></maintain-info>
      </el-dialog>
       <!-- ------------------------------设备监控窗口-------------------------------------------- -->
      <!-- <el-dialog :title="chioseVillage.name" class="maintainDialog" :visible.sync="toolsDialog.devMonitorTab" width="870px" v-if="toolsDialog.dialogDestroy"   @close="toolsDialog.dialogDestroy=false">
      </el-dialog> -->
...
export default {
  ...
  data() {
    return {
      toolsDialog: {
        infoTab: false,   //信息窗口
        maintain: false,    //维修记录窗口
        // devMonitorTab: false,   //设备监控窗口
        sludge: false,      //污泥外送窗口
        dialogDestroy: false    //利用v-if控制dialog的生命周期
      },
  ...
  methods: {
    ...
    searchVillages(ev, pageNumber) {    //搜索
    ...
    handleCurrentChange(pageNumber) {     //搜索列表换页
  ...
  mounted() {
    ...
    sessionStorage.setItem('crumbTitle','运行监控');
  }
};
</script>
<style>
