        ...
        methods: {
            ...
            villageSacda (val) {
                this.$http('/scadas',{
                    ...
                    this.goScadaPage(res.result.rows,'村庄',val.name,0,val.id,'CheckVillageScada');    
                });
            ...
            waterSacda (val) {
                this.$http('/scadas',{
                    ...
                    this.goScadaPage(res.result.rows,'供水设备',val.name,1,val.id,'CheckWaterScada');      
                });
            },
            sewageSacda (val) {
                this.$http('/scadas',{
                    ...
                    this.goScadaPage(res.result.rows,'污水处理设备',val.name,2,val.id,'CheckSewageScada');    
                });
            },
            goScadaPage(data,msg,name,type,id,comp) {
                if (data.length === 0) {
                    this.$message({
                        //message: msg,
                        message: `该村庄暂无${msg}组态，请先配置！`,
                        type: 'warning',
                    });
                }else if (data.length === 1) {
                    sessionStorage.setItem('village_name',name);  
                    sessionStorage.setItem('scada_type',type);
                    this.$router.push({ name: comp, params: { villageId: id } });  
                }
            }
        },
        mounted () {
            ...
            this.getVillages();
            sessionStorage.setItem('crumbTitle','设备监控');
        }
    };
</script>
